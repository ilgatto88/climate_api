...
async def test_get_municipalities(client: AsyncClient, mocker: MockFixture):
    mocker.patch(
        "src.municipality.municipality_db.fetch_all_municipalities",
        return_value=[
...
async def test_get_municipality_by_id(client: AsyncClient, mocker: MockFixture) -> None:
    mocker.patch(
        "src.municipality.municipality_db.fetch_municipality_by_id",
...
def mock_jwt_bearer(mocker: MockFixture):
    mocker.patch(
        "src.municipality.municipality_router.JWTBearer.__call__", return_value=None
    )
...
async def test_post_municipality(
    ...
    mocker.patch(
        "src.municipality.municipality_db.create_municipality",
...
async def test_update_one_municipality(
    ...
    mocker.patch(
        "src.municipality.municipality_db.update_municipality",
        return_value={"m_id": ID, "name": NAME, "state": STATE},
    )
...
async def test_delete_one_municipality(
    ...
    mocker.patch(
        "src.municipality.municipality_db.remove_municipality",
        return_value={"m_id": ID, "name": NAME, "state": STATE},
    )
